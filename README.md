...
* §4.4.2/4.4.3 — the effect-side relation `≈` (`State.Approx`); every
  ...
  every non-`O-Remove` rule (`Step.edit_approx_psi_of_ne_remove`); and the
  resolution coherence core of Theorem 64: in an iteration-only initial
  interval the committed view is fixed and every `L-Iter`/`L-Finish` runs
  against the same resolution `v`; and a lifecycle step from `loading` is
  one of `L-Iter`/`L-Finish`/`L-Raise`/`L-Divert` (`Step.kind_of_loading_lifecycle`).
