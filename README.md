...
| `LeanCordix.Equivariance` | §4.4 | Lemma 56 support: injective renaming of names across views, lifecycle states, fibers, registries, and states, with commutation of `lookup`, `sigmaOf`, `providerOf`, `targetOf`, `relied`, and forward/backward transport of every `Step` record (`step_rename`, `step_rename_bwd`). |
