...
| `LeanCordix.Termination` | §4.4.4 | Termination scaffolding for Thm 66.2: `Step.StepTrace.length`, `countFor`, and `targetTurns` (Eq. 61), with `countFor_eq_zero_of_no_steps`, `countFor_le_length`, and `targetTurns_le_length`; **interval splitting** (`append`, `SourceTargetConst`, `Split`, `splitFirstInterval`), and the **combinatorial half of Theorem 66.2** (`countFor_le_mul_of_interval_bound`, `countFor_le_targetTurns_of_len`): if every source-constant interval has at most `B` steps on a fiber, then `countFor ht n ≤ B * (targetTurns ht n + 1)`. |
...
* §4.4.4 — termination scaffolding: `length`, `countFor`, `targetTurns`,
  ...
  `countFor_le_targetTurns_of_len`.
