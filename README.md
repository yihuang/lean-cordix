...
* §4.3/4.4 — full ten-rule calculus with iterators and failure; full
  ...
  (Thm 59); preservation of `ConfinedWellFormed` under all ten rules;
  full progress (Thm 66.1) under acyclicity of precedence plus the
  confinement-derived invariants; quiescence soundness.

Left as future work: the global metatheory built on confinement
(§4.4.2–4.4.5: recovery exactness, resolution coherence, termination,
confluence).  The confinement invariants themselves are now packaged and
preserved (`ConfinedWellFormed`), and full progress (Thm 66.1) is proved
under acyclicity plus those invariants; termination (Thm 66.2) and
confluence (Thm 73) remain open.
